...
class JSResourceLocator extends ResourceLocator {
	...
	public function doFind($script) {
		...
		if (strpos($script, '/l10n/') !== false) {
			...
			$found = 0;
			$found += $this->appendOnceIfExist($baseDirectory, 'core/'.$script.'.js', $webroot);
			$found += $this->appendOnceIfExist($baseDirectory, $themeDirectory.'/core/'.$script.'.js', $webroot);
			$found += $this->appendOnceIfExist($baseDirectory, $script.'.js', $webroot);
			...
			|| $this->appendOnceIfExist($baseDirectory, $script.'.js', $webroot)
			|| $this->appendOnceIfExist($baseDirectory, $themeDirectory.'/core/'.$script.'.js', $webroot)
			|| $this->appendOnceIfExist($baseDirectory, 'core/'.$script.'.js', $webroot)
		) {
			return;
		}
